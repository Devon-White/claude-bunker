...
```go
fw := container.ContainerFirewallPath // e.g. "/usr/local/bin/init-firewall.sh" — grep the real const
refresh := container.ContainerRefreshFirewallPath
domains := "${containerWorkspaceFolder}/.devcontainer/allowed-domains.txt"
dc["postStartCommand"] = fmt.Sprintf("sudo %s %s && sudo %s %s >/dev/null 2>&1 &", fw, domains, refresh, domains)
```
(Verify the firewall/refresh script paths against `internal/container/constants.go`; verify `${containerWorkspaceFolder}` is the right VS Code substitution for the in-container workspace path. If a user `cfg.PostStartCommand` also exists, append it AFTER the firewall command.) Keep emitting `cfg.Features` (apt-packages) into `features`.
