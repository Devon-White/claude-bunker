...
`claude-bunker init` also targets the VS Code/Codespaces path via two OCI Dev Container Features, sourced under `features/src/`: `firewall` (`ghcr.io/Devon-White/claude-bunker/firewall`) ports the iptables firewall scripts + builtin domain allowlist, and `hardening` (`ghcr.io/Devon-White/claude-bunker/hardening`) installs bubblewrap and sets `securityOpt: apparmor=unconfined`. The custom seccomp profile can't be embedded in a Feature (the spec's `seccomp` value is a host-resolved path), so it's applied via `runArgs: ["--security-opt", "seccomp=${localWorkspaceFolder}/.devcontainer/seccomp.json"]` (`internal/devcontainer/generate.go`) plus a sibling `.devcontainer/seccomp.json` written by `init` from `container.SeccompProfileJSON()` — the same profile bunker's native container creation uses. `init` also references the standard `ghcr.io/devcontainers/features/common-utils:2` Feature (also added via `GenerateOpts.CommonUtilsFeature` in `internal/devcontainer/generate.go`), configured to create the `claude-bunker` user (uid/gid 1000) with passwordless sudo — the base image (`mcr.microsoft.com/devcontainers/base:debian`) only ships a `vscode` user, so without this, `remoteUser: claude-bunker` would fail user resolution on the VS Code/Codespaces path; passwordless sudo is also what the firewall Feature's `postStart` needs. None of these portable artifacts are hand-maintained: `cmd/genfeatures` derives the firewall Feature's scripts and domain list from the embedded scripts and `container.BuiltinDomains()`, and `features/firewall_drift_test.go` fails CI if the packaged copies ever diverge from source. On its own build path, bunker strips all four features (`claude-code`, `firewall`, `hardening`, `common-utils`) from `ProjectConfig` via `bunkerManagedFeaturePrefixes` in `internal/devcontainer/load.go` and ignores `runArgs` entirely, so nothing double-applies — bunker's native Dockerfile creates the `claude-bunker` user itself (the constant `container.ContainerUser`), and native runs never read the file's `remoteUser`. The Features are published to GHCR from `features/src/` only by the opt-in `.github/workflows/publish-features.yml` (manual dispatch or a `features-v*` tag) — never as part of the normal `v*` release path.
