...
| `capAdd` / `remoteUser` | Forced by `init` to `NET_ADMIN`/`NET_RAW` and `claude-bunker` — the firewall requires these regardless of which path builds the container. On the VS Code / Codespaces path, the `claude-bunker` user named by `remoteUser` is created by the `common-utils` feature (see below); bunker's own build creates it natively in its Dockerfile and ignores `remoteUser` entirely (it always runs as the constant `claude-bunker` user). |
...
Because configuration lives entirely in a standard `.devcontainer/devcontainer.json`, a project generated by `claude-bunker init` also opens directly in VS Code's Dev Containers extension or GitHub Codespaces — not just through the `claude-bunker` binary. `init` wires up four things so that path is secure — and actually works — not just convenient:
...
| `claude-bunker` user | The base image (`mcr.microsoft.com/devcontainers/base:debian`) only has a `vscode` user, not `claude-bunker` — so `remoteUser: claude-bunker` alone would fail user resolution when VS Code/Codespaces builds the container. The standard [`common-utils`](https://github.com/devcontainers/features/tree/main/src/common-utils) Feature (`ghcr.io/devcontainers/features/common-utils:2`) creates the `claude-bunker` user (uid/gid `1000`, matching bunker's native base image) and grants it passwordless sudo, which the firewall Feature's `postStart` step needs. Bunker's own build strips this feature (`bunkerManagedFeaturePrefixes` in `internal/devcontainer/load.go`) and creates the same user natively in its Dockerfile instead, so nothing double-applies. |

`claude-bunker init` writes all of this for you (the three features under `features`, the `runArgs` pair, and `.devcontainer/seccomp.json`). **Commit all three files** — `devcontainer.json`, `devcontainer-lock.json`, and `seccomp.json` — so the same protections apply whether a teammate opens the repo through `claude-bunker`, VS Code, or Codespaces. The user-creation mechanism above is in place and covered by tests, but a live VS Code / Codespaces open has not yet been exercised manually — treat that as the remaining validation step before relying on the portable path in anger.
