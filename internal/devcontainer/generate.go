...
type GenerateOpts struct {
	...
	CommonUtilsFeature string // OCI ref for the common-utils feature; "" to omit
}
...
func Generate(cfg config.ProjectConfig, opts GenerateOpts) ([]byte, error) {
	...
	if opts.CommonUtilsFeature != "" {
		// Creates the claude-bunker user (uid/gid 1000, matching bunker's
		// native base image) so remoteUser: claude-bunker actually resolves
		// on the VS Code / Codespaces path — that base image otherwise only
		// has a "vscode" user. common-utils also grants the created user
		// passwordless sudo, which the firewall Feature's postStart needs.
		// Bunker's own build strips this feature (see
		// bunkerManagedFeaturePrefixes in load.go) and creates the user
		// natively in its Dockerfile instead.
		features[opts.CommonUtilsFeature] = map[string]any{
			"username":        bunkerUser,
			"userUid":         "1000",
			"userGid":         "1000",
			"installZsh":      false,
